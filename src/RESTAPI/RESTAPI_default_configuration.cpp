...
namespace OpenWifi {
	...
	void RESTAPI_default_configuration::DoPost() {
		...
		if(DefConfig.platform != Platforms::AP && DefConfig.platform != Platforms::SWITCH && DefConfig.platform != Platforms::OLG) {
			return BadRequest(RESTAPI::Errors::MissingOrInvalidParameters);
		}
	...
	void RESTAPI_default_configuration::DoPut() {
		...
		if(ParsedBody_->has("platform")) {
			if(NewConfig.platform.empty() || (NewConfig.platform != Platforms::AP && NewConfig.platform != Platforms::SWITCH && NewConfig.platform != Platforms::OLG)) {
				return BadRequest(RESTAPI::Errors::MissingOrInvalidParameters);
			}
